...
| F12 | Second worked example (the multi-doc program) and a `context.md` for this paper. | — | **partially applied** (v4) — [context.md](context.md) now exists, and §5.5 notes the worked example as forthcoming; the example itself stays open until the program it would draw on is ready to be public. |
...
**Open.** F12 is the one remaining item, now narrowed: [context.md](context.md) exists and §5.5 announces the worked example as forthcoming, so what remains is the example itself, held until the program it would draw on (currently private) is ready to be shared publicly. Two judgment calls from the v4 pass that the author may want to revisit: the scratch-document practice was added as a *middle form* inside §5.1's two-tier scheme rather than restructuring the scheme into three tiers, and the PDF-export practice was kept to one clause in §5.5 rather than given its own subsection.
